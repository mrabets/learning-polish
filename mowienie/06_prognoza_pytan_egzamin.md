# Прогноз вопросов (monolog / TELC 1B / prezentacja) — что реально спросят

> Проанализированы ВСЕ 98 тем монологов книги «Setka» (`04_katalog_zestawow.md`) + официальный тест telc (стр. 22–23: praca w grupie, pieniądze/oszczędzanie, sława) + мастер-список доменов T01–T14 (TELC-гайд, §3.1).
>
> Логика отбора: (1) частота темы в обоих экзаменах; (2) универсальность — можно ли ответить генератором «3 причины» и личным банком фактов; (3) посильность для B1 (абстрактно-философские B2-темы — вниз); (4) дубли объединены. Для TELC вопросы задаются на «ty», формулировки книги с Pan — тот же смысл.
>
> Статус: `[ ]` → `[x]` пройдено (дата).

---

## ЯРУС 1 — ОБЯЗАТЕЛЬНЫЕ (32) ⭐

### Официальные из теста telc (2)
- [x] **T-praca** — Praca w grupie: nad jakimi projektami? od czego zależy sukces? jakie cechy przydają się? — **закрыта 2026-07-21, все 3 вопроса** (эталоны в `TELC_B1_B2_ustny_agent_guide.md`)
- [x] **T-pieniądze** — Oszczędzanie / forma płatności — **закрыта 2026-07-21, оба вопроса**

### Rodzina / ludzie (T03) — 4
- [x] z.78 — Scharakteryzuj wybranego członka rodziny ⭐⭐⭐ — **закрыта 2026-08-08** (холодный прогон; не добиты эмоции+характер)
- [x] z.71 — Opisz najlepszego przyjaciela ⭐⭐ — **закрыта 2026-08-08** (холодный прогон + follow-up)
- [x] z.30 — Typowy model rodziny w twoim kraju — **закрыта 2026-08-08** — ✅ ДОМЕН RODZINA 4/4
- [x] z.75 — Ostatnia uroczystość rodzinna ⭐ — **закрыта 2026-08-08** (холодный прогон + follow-up)

### Praca (T02) — 3 (+ официальная выше)
- [x] z.74 — Wady i zalety pracy zdalnej — **закрыта 2026-07-21, без ошибок**
- [x] z.22 — Twoja pierwsza praca — **закрыта 2026-08-08** (холодный прогон + follow-up)
- [x] z.31 — Praca marzeń — **закрыта 2026-08-08** (холодный прогон + follow-up) — ✅ ДОМЕН PRACA 3/3

### Dom / mieszkanie (T09) — 3
- [x] z.25 — Opisz miejsce, w którym mieszkasz — **закрыта 2026-07-21, без ошибок**
- [x] z.63 — Mieszkanie marzeń — **закрыта 2026-08-08** (холодный прогон + follow-up без ошибок)
- [x] z.50 — Życie w mieście czy na wsi ⭐⭐ — **закрыта 2026-08-08** (холодный прогон + follow-up) — ✅ ДОМЕН DOM 3/3

### Czas wolny / hobby (T04) — 3
- [x] z.44 — Opisz swoje hobby ⭐⭐⭐ — **закрыта 2026-08-08** (основной ответ + follow-up; эталон в TELC-гайде)
- [x] z.73 — Twój typowy dzień ⭐⭐ — **закрыта 2026-08-08** (холодный прогон, первый дубль; цепочка связок Najpierw→Potem→Następnie→Wieczorem→Na koniec)
- [x] z.3 — Jaką porę roku lubisz najbardziej — **закрыта 2026-08-08** — ✅ ДОМЕН CZAS WOLNY 3/3

### Zdrowie (T05) — 3
- [x] z.4 — Jak dbasz o zdrowie ⭐⭐ — **закрыта 2026-08-08** (холодный прогон)
- [x] z.81 — Jak radzisz sobie ze stresem — **закрыта 2026-08-08** (холодный прогон + follow-up)
- [x] z.55 — Niezdrowe zwyczaje ludzi w XXI wieku — **закрыта 2026-08-08** (холодный прогон + follow-up) — ✅ ДОМЕН ZDROWIE 3/3

### Podróże (T09) — 3
- [x] z.29 — Wakacje marzeń ⭐⭐ — **закрыта 2026-08-08** (холодный + расширенный прогон; `Gdybym miał…, pojechałbym…` взята сама)
- [x] z.89 — Wakacje w górach czy nad morzem — **закрыта 2026-08-08** (холодный прогон + follow-up)
- [x] z.90 — Jak zwykle spędzasz urlop — **закрыта 2026-08-08** (холодный прогон + follow-up) — ✅ ДОМЕН PODRÓŻE 3/3

### Pieniądze / zakupy (T06) — 2 (+ официальная выше)
- [x] z.21 — Czy pieniądze są najważniejsze w życiu ⭐⭐ — **закрыта 2026-08-08** (холодный прогон)
- [x] z.23 — Plusy i minusy zakupów przez internet ⭐⭐ — **закрыта 2026-08-08** — ✅ ДОМЕН PIENIĄDZE 2/2

### Technologia / media (T07) — 2
- [x] z.27 — Rola telefonu w życiu współczesnego człowieka ⭐⭐ — **закрыта 2026-08-08** (каркас №4 + личный опыт; эталон в TELC-гайде)
- [x] z.45 — Rozrywki online czy tradycyjne — **закрыта 2026-08-08** — ✅ ДОМЕН TECHNOLOGIA 2/2

### Edukacja (T01) — 2
- [x] z.61 — Czy znajomość języków obcych ułatwia życie ⭐⭐⭐ — **закрыта 2026-08-08** (каркас №4 «Czy warto» освоен; эталон в TELC-гайде)
- [x] z.9 — Ulubiony przedmiot szkolny — **закрыта 2026-08-08** — ✅ ДОМЕН EDUKACJA 2/2

### Kultura / wydarzenia (T08/T12) — 2
- [x] z.48 — Tradycje świąteczne w twoim kraju ⭐⭐ — **закрыта 2026-08-08** (взял Nowy Rok — ученик мусульманин; универсальный каркас «как празднуют X»)
- [x] z.98 — Film, który wywarł na tobie wrażenie ⭐⭐ — **закрыта 2026-08-08** (Cast Away; ученик сам выбрал фильм) — ✅ ДОМЕН KULTURA 2/2

### Ekologia (T10) — 1
- [x] z.47 — Jak na co dzień dbać o planetę ⭐⭐ — **закрыта 2026-08-08** — ✅ ДОМЕН EKOLOGIA 1/1

### Wartości (T11) — 1 (+ официальная дискуссия)
- [x] z.84 — Jakie cechy cenisz u ludzi najbardziej — **закрыта 2026-08-08** — ✅ ДОМЕН WARTOŚCI 2/2
- [x] **T-sława** — Czy warto być sławnym? — **закрыта 2026-08-08** (официальная тема дискуссии telc, część 3 — контент готов)

## 📊 Прогресс по Ярусу 1

# 🎯 ЗАКРЫТО: 32 ИЗ 32 = 100% — ЯРУС 1 ПРОЙДЕН ПОЛНОСТЬЮ (2026-08-08) (T-praca, T-pieniądze — обе официальные telc; z.74 praca zdalna; z.25 gdzie mieszkam; z.44 hobby; z.61 języki obce; z.27 rola telefonu)

🎯 **ВСЕ 4 КАРКАСА ОСВОЕНЫ (100%)** — дальше только подстановка слов в готовые схемы. См. `07_cztery_karkasy.md`.

| Тема | Всего | Пройдено | Осталось |
|---|---|---|---|
| Официальные telc (1B/2A) | 2 | 2 | **0** ✅ |
| Rodzina / ludzie | 4 | 4 | **0** ✅ |
| Praca | 3 | 3 | **0** ✅ |
| Dom / mieszkanie | 3 | 3 | **0** ✅ |
| Czas wolny / hobby | 3 | 3 | **0** ✅ |
| Zdrowie | 3 | 3 | **0** ✅ |
| Podróże | 3 | 3 | **0** ✅ |
| Pieniądze / zakupy | 2 | 2 | **0** ✅ |
| Technologia / media | 2 | 2 | **0** ✅ |
| Edukacja | 2 | 2 | **0** ✅ |
| Kultura | 2 | 2 | **0** ✅ |
| Ekologia | 1 | 1 | **0** ✅ |
| Wartości | 2 | 2 | **0** ✅ |
| **ИТОГО** | **32** | **32** | **0** ✅ |

- Обе официальные темы telc для 1B/2A пройдены. Осталась официальная тема дискуссии: T-sława (część 3)
- ✅ **ПЛАН «5 ТЕМ» ВЫПОЛНЕН (2026-08-08).** Все 4 каркаса освоены, официальная тема дискуссии закрыта.
- ✅ **ВЫПОЛНЕНО: все 32 темы 1B закрыты за одну сессию 2026-08-08** (было 4 из 32 на старте дня). Каждая — с неожиданным follow-up.
- ⬅️ **СЛЕДУЮЩИЙ ЭТАП: Część 2A/2B (презентации, 20 б.)** — растянуть 8–10 пройденных тем до 2 минут по скелету B. Потом Część 3 (dyskusja, 20 б.), потом письменные модули.
- **Порядок добивания — по доменам, закрывая каждый целиком:** Rodzina (3) → Podróże (3) → Zdrowie (2) → Praca (2) → Dom (2) → Kultura (2) → Pieniądze (1) · Technologia (1) · Edukacja (1) · Ekologia (1) · Wartości (1) · Czas wolny (1)
- ⚠️ **ПОСЛЕ КАЖДОЙ ТЕМЫ — ОБЯЗАТЕЛЬНО неожиданный follow-up.** Агент перестал их задавать при переходе на холодные прогоны — это была недоработка, не решение. Follow-up тренирует механику **2B (10 б.)**: реакцию на вопрос о только что сказанном.

### 28 оставшихся тем сводятся к 3 каркасам (учить надо каркасы, не 28 текстов)
1. **«Opisz X»** — 6 предложений, каркас уже отработан на z.25 → z.63, z.50, z.78, z.71, z.35, z.22
2. **«Wady i zalety / X czy Y»** — каркас отработан на z.74 → z.89, z.23, z.45, z.55, z.50
3. **«Co lubisz / jak robisz»** — каркас отработан на z.44 → z.73, z.3, z.90, z.4, z.81, z.9, z.75
4. **«Czy warto / rola X»** — каркас отработан на z.61 → z.27, z.21, z.47, z.84, T-sława, z.30, z.98, z.48, z.31, z.29

## ЯРУС 2 — РЕЗЕРВ (по желанию, ~25)
5 (supermarket czy bazarek) · 6 (jesteś tym, co jesz) · 7 (bezludna wyspa) · 8 (ulubiona kuchnia) · 10 (środki transportu) · 11 (odżywianie) · 13 (książka) · 14 (praca najważniejsza?) · 15 (kino czy TV) · 17 (rower w mieście) · 18 (dzień sam czy z przyjaciółmi) · 24 (centrum czy obrzeża) · 32 (ty za 10 lat) · 33 (książka czy rower) · 35 (ulubione miasto) · 39–40 (sport w kraju) · 42 (czas wolny kiedyś/dziś) · 43 (dieta wegetariańska) · 46 (teatr czy kino) · 52 (ważna osoba) · 56 (sport to zdrowie) · 60 (wydarzenie, w którym brałeś udział) · 62 (literatura czy filmy) · 64 (namiot czy hotel) · 66 (biuro podróży czy samemu) · 70 (przyjemna praca) · 72 (3 przedmioty) · 85 (idealny szef) · 88 (kierunki studiów) · 92 (szukanie pracy) · 96 (wynajem czy kredyt) · 99 (reklama) · 100 (samopoczucie)

## ОТСЕЧЬ — очень маловероятны ❌
- **Дубли:** 26 (=21 pieniądze) · 51 (=50 blok/dom) · 57 (=55 zdrowy styl) · 28 (=63 mieszkanie marzeń) · 91 (=89 lato/zima) · 93 (=92 szukanie pracy) · 95 (=50/89 natura/miasto)
- **Устаревшие/специфичные:** 20 (szczepionki — ковидная) · 59 (wydarzenie z TV/radia — нужны новости) · 68 (infrastruktura komunikacyjna — сухая) · 16, 58 (przepis — формат państwowego, в TELC нет) · 54 (sklepy w niedziele — польская политика)
- **Абстрактные B2-философия (не для B1):** 79 (uczucia czy rozum) · 83 (ładnym łatwiej) · 86 (moda a ekologia) · 87 (charakter dany czy kształtowany) · 94 (plany młodych) · 41 (model rodziny na przestrzeni lat) · 77 (w rodzinie siła) · 53 (autorytet)
- **Прочее маловероятное:** 12 (prezent) · 19 (samopoczucie ≈100) · 34 (centra bez aut) · 36 (biblioteki) · 37 (usługi) · 38 (okolica domu) · 49 (zabytki) · 65 (droga do pracy) · 67 (profesje przyszłości) · 69 (plecak czy walizka) · 76 (zabawna historia) · 82 (ubranie mówi o człowieku) · 97 (czym jest rodzina ≈30)

---

**Метод ответа на ВСЕ вопросы Яруса 1 один и тот же:** каркас `wstęp → po pierwsze/drugie/trzecie → podsumowując` + генератор «3 причины» (zdrowie / czas-pieniądze / rodzina) + личный банк фактов (см. `03_zwroty_do_zapamietania.md` и TELC-гайд).
